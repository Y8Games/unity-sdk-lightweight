mergeInto(LibraryManager.library, 
{
                ...
                ID.Event.subscribe('id.init', function()
                    {
						...
						// second level of init (from example at http://onefoldmedia.com/idnet3.html#)
						ID.GameAPI.init(appId, null, function(data, response)
							{
								//console.log(data, response);
								// tell c# that the system is fully ready
								SendMessage('LoginY8', 'CallbackReady');
							});
                    });
				...
				ID.Event.subscribe('auth.authResponseChange', function(auth)
					{
						...
						SendMessage('LoginY8', 'AuthCallbackResponse', JSON.stringify(auth));
					});
    ...
    Call: function( _id, _request, _jsonData )
    {
        ...
        var idCallback = function(response)
        {
            ...
            {	// the server processed the response
				...
				if (request != 'login' && request != 'auto_login')
				{
					...
					SendMessage('LoginY8', 'CallbackResponse', request + '[' + _id + ']=' + json);
				}
            }
			else
			{
				SendMessage('LoginY8', 'CallbackResponse', request + '[' + _id + ']=' + '{}');
			}
        }
        ...
        var idCallbackData = function(data, response)
        {
			...
			{
				//console.log("JS " + data);
				SendMessage('LoginY8', 'CallbackResponse', request + '[' + _id + ']=' + JSON.stringify(data));
			}
            if (response)
            {
                //console.log("JS " + response);
				SendMessage('LoginY8', 'CallbackResponse', _id, request + '[' + _id + ']=' + JSON.stringify(response));
            }
        }
